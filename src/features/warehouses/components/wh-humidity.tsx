...
// Generate fake humidity data for 3 cells
const generateFakeData = (
  ...
  const data: { date: string; cell1Humidity: number; cell2Humidity: number; cell3Humidity: number }[] = [];
  ...
  for (let i = 0; i < numPoints; i++) {
    const currentDate = timeIncrement(startDate, i);

    // Generate random humidity between 30% and 80% for each cell
    const cell1Humidity = Math.round(30 + Math.random() * 50);
    const cell2Humidity = Math.round(30 + Math.random() * 50);
    const cell3Humidity = Math.round(30 + Math.random() * 50);
...
export default function HumidityLineChart({ warehouseId }: { warehouseId: string }) {
      ...
      <CardHeader className="flex flex-col items-start justify-between border-b border-gray-100 p-6 sm:flex-row sm:items-center">
        <div className="flex flex-col gap-2">
          ...
          <CardDescription className="text-gray-500">
            Humidity (%) across all cells in Warehouse {warehouseId}
          </CardDescription>
        </div>
      </CardHeader>
            ...
            {data.length === 0 ? (
              ...
              >
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => [`${value}%`, name.replace('Humidity', '')]}
                  labelFormatter={formatTooltipLabel}
                />
                <Legend formatter={(value) => value.replace('Humidity', '')} />
                <Line
                  type="monotone"
                  dataKey="cell1Humidity"
                  name="Cell 1"
                  stroke="#16a34a"
                  strokeWidth={2}
                  dot={false}
                ...
                <Line
                  type="monotone"
                  dataKey="cell2Humidity"
                  name="Cell 2"
                  stroke="#d946ef"
                  strokeWidth={2}
                  dot={false}
                  activeDot={{ r: 6, stroke: '#e879f9', strokeWidth: 2, fill: '#fff' }}
                />
                <Line
                  type="monotone"
                  dataKey="cell3Humidity"
                  name="Cell 3"
                  stroke="#2563eb"
                  strokeWidth={2}
                  dot={false}
